...
async function numberFact() {
  let response = await $.getJSON(`${baseURL}/${favNumber}/trivia?json`);
...
async function numbersFacts() {
  let response = await $.getJSON(`${baseURL}/${nums}/trivia?json`);
  console.log(response);
...
async function zeroFacts() {
  let facts = await Promise.all(
    Array.from({length: 4}, () => $.getJSON(`${baseURL}/${favNumber}/trivia?json`))
  );
  console.log(facts);
